...
export default function LiveMap({ positions }: LiveMapProps) {
  ...
  useEffect(() => {
    ...
    return () => { socket.off('vehicle:position', handlePositionUpdate); };
  }, [socket, queryClient]);
          ...
          >
            <Popup className="dark-popup">
              <div className="p-1">
                ...
                <div className="text-xs mt-2 font-mono">Vel: {pos.speed} km/h</div>
                <div className="text-xs font-mono">Ign: {pos.ignition ? 'Ligada' : 'Desligada'}</div>
              </div>
            </Popup>
          </Marker>
        ))}
