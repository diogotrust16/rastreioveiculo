...
export default function Vehicles() {
  ...
  async function handleSave() {
    const payload = { plate: form.plate, model: form.model, imei: form.imei, clientId: form.clientId ? parseInt(form.clientId) : undefined };
    if (dialog === 'create') {
            ...
            <tbody>
              {filtered.map((v, i) => (
                <tr key={v.id} className={`border-b border-border/50 hover:bg-muted/20 transition-colors ${i % 2 === 0 ? '' : 'bg-muted/5'}`}>
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{v.clientName ?? '—'}</td>
