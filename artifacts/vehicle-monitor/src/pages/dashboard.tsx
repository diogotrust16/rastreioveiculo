...
import { Target, AlertTriangle, Car, Users, Activity, CheckCircle2 } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const { data: positions = [], isLoading: posLoading } = useGetLivePositions();

  return (
    <div className="flex-1 p-6 overflow-y-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-primary">System Dashboard</h1>
          <p className="text-muted-foreground text-sm">Real-time overview of fleet operations.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[600px]">
        <div className="lg:col-span-2 bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col">
          <div className="p-4 border-b border-border flex justify-between items-center bg-secondary/30">
            <h2 className="font-semibold tracking-wide">Live Map</h2>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
              <span className="text-xs font-mono text-primary/80">LIVE</span>
            </div>
          </div>
        ...
        <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col">
          <div className="p-4 border-b border-border bg-secondary/30 flex justify-between items-center">
            <h2 className="font-semibold tracking-wide">Recent Alerts</h2>
